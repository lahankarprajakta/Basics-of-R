...
```{r echo=FALSE}
setwd('C:/Users/praja_m3gddx7/Desktop/Lubin_Bootcamp/')
```
...
str(wages)
library(ggplot2)
ggplot(data=wages, aes(x=earn)) + geom_histogram(binwidth = 5000)
...
1 - sse2 /sst2
rmse2 = sqrt(mean((pred - wages$earn)^2))
rmse2
```
`
## Inference  
...
1 - sse3 /sst3
rmse3 = sqrt(mean((pred - wages$earn)^2))
rmse3
```
...
## Estimate
```{r, warning=F,message=F}
model6 = lm(earn~height+gender+race+ed+age,wages)
```
...
1 - sse6 /sst6
rmse6 = sqrt(mean((pred - wages$earn)^2))
rmse6
```
...
model = lm(earn~.,wages)
pred = predict(model)
rmse = sqrt(mean((pred-wages$earn)^2));rmse
```
